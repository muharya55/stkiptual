    ...
    posts.map((post) => (
      <div key={post.slug} className="latest-posts d-flex gap-20px mb-20px">
        <div className="latest-posts_thumb" >
          <Link href={`/berita-detail/${post.slug}`} target="_blank" rel="noopener noreferrer">
            <Image
        ...
        <div className="latest-posts_content">
          <h6 className="fs-15 fw-semibold line-clamp-2 mb-10px">
            <Link href={`/berita-detail/${post.slug}`} target="_blank" rel="noopener noreferrer" className="transition-all fw-semibold">
              {post.judul}
            </Link>
          </h6>
